...
class ApiController extends Controller {
    ...
    public function actionList() {
            ...
            switch($_GET['model']) {
                ...
                case 'value':
                    if (isset($_GET["value"]) or $_GET["value"]!=""){
                        $data = new Data;
                        $data->sensor=urldecode($_GET["sensor"]);
                        $data->value=urldecode($_GET["value"]);
                        if ($data->save()){
                            $models=array('ok'=>'1');
                        }else{
                            $models=array('ok'=>'0');
            ...
            if(empty($models)) {
                ...
                // Prepare response
                $this->_sendResponse(200, NJSON::encode($models));  
...
/*                $rows = array();
    ...
    // Check if id was submitted via GET
        ...
        switch($_GET['model'])
        {
            ...
            case 'data':
                $model = Data::model()->findByPk($_GET['id']);  
                break;
            default:
                $this->_sendResponse(501, sprintf(
        ...
        if(is_null($model))
            ...
            $this->_sendResponse(200, NJSON::encode($model,true));
    }
    public function actionCreate() {
        ...
        if($model->save())
            $this->_sendResponse(200, NJSON::encode($model));
        else {
            // Errors occurred
            $msg = "<h1>Error</h1>";
    ...
    public function actionUpdate() {
        // Parse the PUT parameters. This didn't work: parse_str(file_get_contents('php://input'), $put_vars);
            ...
            $put_vars = NJSON::decode($json,true);  //true means use associative array
    ...
    private function _checkAuth() {
        ...
        // Find the user
        $user=Users::model()->find('LOWER(name)=?',array(strtolower($username)));
        if($user===null) {
            // Error: Unauthorized
