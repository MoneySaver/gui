 ...
 * @since Jun 19, 2013
 */
class NJSON extends CJSON{
	public static function encode($var,$show_null=false){
		switch (gettype($var)) {
			case 'boolean':
			...
			case 'array':
				...
				*/
				...
				if (is_array($var) && count($var) && (array_keys($var) !== range(0, sizeof($var) - 1))) {
					return '{' .
						   join(',', array_map(array('NJSON', 'nameValue'),
											   array_keys($var),
											   array_values($var)))
						   . '}';
				}
				...
				return '[' . join(',', array_map(array('NJSON', 'encode'), $var)) . ']';
				...
				if ($var instanceof CActiveRecord){
					foreach ($var->relations() as $key => $related){
						if ($var->hasRelated($key)){
							if(is_array($var->$key) && count($var->$key)){
								if ($var->$key!=null or $show_null){
									$vars[$key] = self::encode($var->$key);
								}
							}elseif ($var->$key!=null){
								$vars[$key] = $var->$key;
							}
